...
import { generateMotion } from "@/src/agents/motion";
...
export async function POST(req: NextRequest) {
  ...
  // Use the first idea for motion by default
  const primary = plan.daily_ideas[0];
  const motion = generateMotion(primary);
